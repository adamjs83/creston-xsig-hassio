...
class EntityManager:
    ...
    async def async_step_select_entity_to_edit(
        ...
        for cl in current_climates:
            ...
            join_display: str | None = cl.get(CONF_FLOOR_SP_JOIN) if climate_type == "floor_warming" else cl.get(CONF_HEAT_SP_JOIN)
            entity_options.append({
        ...
        if user_input is not None:
            try:
                ...
                if entities_to_remove:
                    ...
                    updated_covers: list[dict[str, Any]] = [c for c in current_covers if c.get(CONF_NAME) not in entities_to_remove]
                    updated_binary_sensors: list[dict[str, Any]] = [bs for bs in current_binary_sensors if bs.get(CONF_NAME) not in entities_to_remove]
                    updated_sensors: list[dict[str, Any]] = [s for s in current_sensors if s.get(CONF_NAME) not in entities_to_remove]
                    updated_lights: list[dict[str, Any]] = [l for l in current_lights if l.get(CONF_NAME) not in entities_to_remove]
                    updated_switches: list[dict[str, Any]] = [sw for sw in current_switches if sw.get(CONF_NAME) not in entities_to_remove]
                    updated_climates: list[dict[str, Any]] = [cl for cl in current_climates if cl.get(CONF_NAME) not in entities_to_remove]
                    updated_media_players: list[dict[str, Any]] = [mp for mp in current_media_players if mp.get(CONF_NAME) not in entities_to_remove]
                    ...
                    for entity_name in entities_to_remove:
                        ...
                        if entity_config and entity_type:
                            ...
                            entity_id: str | None = entity_reg.async_get_entity_id(
